    ...
    for (let day = 1; day <= daysInMonth; day++) {
      days.push(
        <span
          ...
          className="calendar-day"
          onClick={(e) => {
            e.stopPropagation(); // Prevent event from bubbling to month container
            onMonthSelect(new Date(selectedYear, monthIndex, day));
          }}
        >
          {day}
        </span>
      ...
      <div className="months-grid">
        {months.map((month, index) => (
          <div 
            ...
            onClick={() => onMonthSelect(new Date(selectedYear, index, 1))}
          >
            <h3 className="month-title">{month}</h3>
            <div className="weekdays">
              <span>S</span>
