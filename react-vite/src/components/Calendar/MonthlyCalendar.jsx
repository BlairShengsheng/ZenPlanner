...
const renderCalendarDays = () => {
  ...
  for (let i = 0; i < firstDay; i++) {
    days.push(
      <div 
        key={`empty-${currentDate.getMonth()}-${i}`} // Updated key to be unique
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        key={`day-${currentDate.getMonth()}-${day}`} // Updated key to be unique
        className={`calendar-cell ${isToday ? 'today' : ''}`}
        onClick={() => handleDayClick(date)}
      >
        {day}
    ...
    <div className="calendar-grid">
      <div className="weekdays">
        {daysOfWeek.map((day, index) => (
          <div key={`weekday-${index}`} className="weekday">{day}</div> // Updated key to be unique
        ))}
      </div>
      <div className="days">
        {renderCalendarDays()}
